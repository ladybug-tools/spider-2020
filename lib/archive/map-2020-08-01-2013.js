...
MAP.init = function () {
	...
	THR.camera.position.set( 0. -120, 150);
	...
	heightmapLatMax = MAP.tile2lat( MAP.tileCenterY - 1, MAP.zoom );
	heightmapLatMin = MAP.tile2lat( MAP.tileCenterY + 1, MAP.zoom );

	heightmapLonMax = MAP.tile2lon( MAP.tileCenterX - 1, MAP.zoom );
	heightmapLonMin = MAP.tile2lon( MAP.tileCenterX + 1, MAP.zoom );
	...
	// latDelta = ( heightmapLatMax - heightmapLatMin );
	// lonDelta = ( heightmapLonMax - heightmapLonMin );

	// latDeltaTarget = latitude - latMin;
	// lonDeltaTarget = longitude - lonMin;

	// if ( latDiff < 0.5 ) { tileOffsetY = 1; }
	// else { titleOffsetY = 0; }
	...
	str = JSON.stringify( MAP, null, "<br>" )

	divStats.innerHTML = `
	<p>
	heightmapLatMax: ${ heightmapLatMax }<br>
	heightmapLatMin: ${ heightmapLatMin }<br>
	<br>
	heightmapLonMax: ${ heightmapLonMax }<br>
	heightmapLonMin: ${ heightmapLonMin }<br>
	</p>
...
MAP.getTilesBitmaps = function () {
	...
	MAP.tileBitmapStartX = MAP.tileBitmapCenterX + deltaBMX - Math.floor( xx * MAP.cols / 2 ) + MAP.deltaX;
	MAP.tileBitmapStartY = MAP.tileBitmapCenterY + deltaBMY - Math.floor( xx * MAP.rows / 2 ) + MAP.deltaY;
