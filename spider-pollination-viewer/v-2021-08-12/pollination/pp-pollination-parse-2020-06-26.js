...
THRR.getHtm = function ( intersected ) {
	...
	console.log( "intersectedHtm", intersected );
		...
		json.rooms.forEach( ( room, index ) => {
			...
			for ( let face of room.faces ) {
				...
				for ( let point of face.geometry.boundary ) {
					...
					thisItem = checkNearest ? room : thisItem;
	...
	if ( thisItem ) {
		...
		butt = thisItem.userData ? `<button onclick="THRR.showFind(${ thisItem.userData.index })" >get room parameters</button>` : "";

		htm = `
			name: ${ thisItem.display_name }<br>
			type: ${ thisItem.type }<br>
			construction set: ${ thisItem.properties.energy.construction_set }<br>
			hvac: ${ thisItem.properties.energy.hvac }<br>
			program type: ${ thisItem.properties.energy.program_type }<br>
			${ butt }
		`;
		return htm;
