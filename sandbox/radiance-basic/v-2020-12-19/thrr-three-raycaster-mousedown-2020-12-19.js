...
THRR.updateScene = function ( objs = THR.group.children ) {

	THRR.intersectObjects = objs.length ? objs : THR.scene.children;
...
THRR.onPointerDown = function ( event ) {
	...
	THRR.intersectObjectsVisible = THRR.intersectObjects.filter( mesh => mesh.visible );

	let intersects = THRR.raycaster.intersectObjects( THRR.intersectObjectsVisible );
...
THRR.getHtm = function ( intersected ) {
	...
	const htm = `
	<div>
		id: ${THRR.intersectObjects.indexOf( mesh ) }<br>
		geometry: ${ mesh.geometry.type }<br>
		name: ${ mesh.name }</br>
		uuid: ${mesh.uuid }<br>
		<button onclick=THRR.getMeshData(${THRR.intersectObjects.indexOf( mesh ) }); >view mesh data</button>
	</div>`;
