...
FO.onHashChange = function() {
	...
	FO.requestFile(FO.url, FO.onLoad);
}


FO.requestFile = function ( url, onLoad = FO.onLoad ) {
	...
	FO.xhr.onload = function( xhr ) { FO.string = xhr.target.response; onLoad( xhr.target.response ); };
	FO.xhr.send( null );
