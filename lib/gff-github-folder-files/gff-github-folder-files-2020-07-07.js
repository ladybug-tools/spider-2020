...
GFF.loadFile = function( index, count ) {
	...
	if ( count < GFF.files[ index ].length ) {
		...
		setTimeout( () => { GFF.loadFile( index, count ); }, 10000 );
...
GFF.callbackGitHubMenu = function ( xhr ) {
	...
	for ( let file of files ) {
		...
		<div style=margin:0.5rem 0; >
			...
			<a href=${ item.threeViewer }#${item.urlGitHubPage}${fileName} title="Link to just this file. Open file in new tab." target="_blank" >
