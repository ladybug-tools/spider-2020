...
FOH.urlDefaultFileXml = "https://cdn.jsdelivr.net/gh/ladybug-tools/spider@master/gbxml-sample-files/bristol-clifton-downs-broken.xml";

FOH.urlDefaultFileJson = "https://cdn.jsdelivr.net/gh/ladybug-tools/dragonfly-schema@master/samples/building_simple.json";

FOH.urlDefaultFileZip = "https://cdn.jsdelivr.net/gh/ladybug-tools/spider@master/gbxml-sample-files/zip/warwick-university-5k-surfaces.zip";

FOH.urlDefaultFileObj = "https://cdn.jsdelivr.net/gh/ladybug-tools/3d-models@gh-pages/quaternius/ultimate-nature-pack/BirchTree_1.obj";
...
FOH.requestFileDecider = function( url ) { // from a button
	...
	if ( FOH.regexHtml.test( url ) ) {
	...
	} else if ( FOH.fileName.toLowerCase().endsWith( '.obj' )) {
		...
		FO.requestFile( url, FOH.callbackDecider );
...
FOH.xxxrequestFileText = function( url ) {
...
FOH.ccconProgress = function( size = 0, note = '' ) {
...
FOH.callbackJson = function( text ) {
	...
	//window.addEventListener( 'onloadJson', () => { console.log( '', text ) }, false );
