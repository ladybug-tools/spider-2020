...
FOO.onHashChange = function() {
	...
	FOO.requestFile(FOO.url, RAD.onLoad);
...
FOO.requestFile = function ( url = urlDefault, onLoad = FOO.onLoadXhr ) {
	//console.log( 'url', url );
	...
	FOO.xhr.onload = ( xhr ) => onLoad( xhr.target.response );
	FOO.xhr.send( null );
...
FOO.onLoadXhr = function ( response ) {
