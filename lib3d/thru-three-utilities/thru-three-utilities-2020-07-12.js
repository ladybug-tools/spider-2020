...
THRU.tellTaleReset = function( mesh ) {

	mesh.remove( THRU.groupTellTale );
...
THRU.nnnnaddLineFace = function( vertices ) { // THRR-caster only

	const geometry = new THREE.Geometry();
	geometry.vertices = vertices;
	const material = new THREE.LineBasicMaterial( { color: 0xffffff } );
	THRU.lineFace = new THREE.Line( geometry, material );
	THRU.groupTellTale.add( THRU.lineFace );
...
THRU.addLine = function( vertices, color = 0x000000 ) { // THRR-caster only

	const geometry = new THREE.Geometry();
	geometry.vertices = vertices;
	const material = new THREE.LineBasicMaterial( { color: color } );
	const line = new THREE.Line( geometry, material );
	THRU.groupTellTale.add( line );
