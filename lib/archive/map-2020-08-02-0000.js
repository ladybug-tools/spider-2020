...
MAP.init = function () {
	...
	deltaX = 0; // ( MAP.cols ) % 2 === 0 ? 0 : 0;
	//console.log( "deltaX ", deltaX );

	deltaY = 0; //( MAP.rows ) % 2 > 0 ? 0 : 0;
	...
	centerLatMax = MAP.tile2lat( MAP.tileCenterY, MAP.zoom );
	centerLatMin = MAP.tile2lat( MAP.tileCenterY + 1, MAP.zoom );

	centerLonMax = MAP.tile2lon( MAP.tileCenterX, MAP.zoom );
	centerLonMin = MAP.tile2lon( MAP.tileCenterX + 1, MAP.zoom );

	latDelta = Math.abs( centerLatMax - centerLatMin );
	lonDelta = Math.abs( centerLonMax - centerLonMin );

	latOffsetTarget = Math.abs( centerLatMax - MAP.latitude );
	lonOffsetTarget = Math.abs( centerLonMax - MAP.longitude );


	latDiff = latOffsetTarget / latDelta;
	lonDiff = lonOffsetTarget / lonDelta;

	if ( latDiff < 0.5 ) { tileOffsetY = 0; }
	else { tileOffsetY = -1; }
	//tileOffsetY = 0;

	if ( lonDiff < 0.5 ) { tileOffsetX = 1; }
	else { tileOffsetX = 0; }
	//tileOffsetX = 0;

	x = -25 + ( lonOffsetTarget / lonDelta ) * 50;
	y = 25 - ( latOffsetTarget / latDelta ) * 50;

	const geometry = new THREE.BoxGeometry( 0.3, 0.3, 1000 );
	...
	MAP.tileStartX = MAP.tileCenterX + deltaX - Math.floor( MAP.cols / 2 );
	MAP.tileStartY = MAP.tileCenterY + deltaY - Math.floor( MAP.rows / 2 );

	MAP.tileBitmapCenterX = MAP.lonToTile( MAP.longitude, 1 + MAP.zoom ) + MAP.deltaX;
	MAP.tileBitmapCenterY = MAP.latToTile( MAP.latitude, 1 + MAP.zoom ) + MAP.deltaY;



	bitmapLatMax = MAP.tile2lat( MAP.tileBitmapCenterY - 2, 1 + MAP.zoom );
	bitmapLatMin = MAP.tile2lat( MAP.tileBitmapCenterY + 2, 1 + MAP.zoom );

	bitmapLonMax = MAP.tile2lon( MAP.tileBitmapCenterX - 3, 1 + MAP.zoom );
	bitmapLonMin = MAP.tile2lon( MAP.tileBitmapCenterX + 1, 1 + MAP.zoom );
	...
	<p>
		bitmapLatMax: ${ bitmapLatMax }<br>
		bitmapLatMin: ${ bitmapLatMin }<br>
	<br>
		bitmapLonMax: ${ bitmapLonMax }<br>
		bitmapLonMin: ${ bitmapLonMin }<br>
	</p>
	<details>
		<p>${ str }</p>
	</details>
...
MAP.getTilesBitmaps = function () {
	...
	xx = 2;
	MAP.canvasBitmap.width = MAP.pixelsPerTile * MAP.cols * xx;
	MAP.canvasBitmap.height = MAP.pixelsPerTile * MAP.rows * xx;
	...
	deltaBMX = 0; //( MAP.cols ) % 2 === 0 ? -1: -2;
	console.log( "deltaXBM", deltaBMX );

	deltaBMY = 1; //( MAP.rows ) % 2 === 0 ? 1 : -1;
	console.log( "deltaBMY", deltaBMY );

	MAP.tileBitmapStartX = MAP.tileBitmapCenterX + deltaBMX + tileOffsetX - Math.floor( xx * MAP.cols / 2 ) + MAP.deltaX;
	MAP.tileBitmapStartY = MAP.tileBitmapCenterY + deltaBMY + tileOffsetY - Math.floor( xx * MAP.rows / 2 ) + MAP.deltaY;
	...
	for ( let x = 0; x < xx * MAP.cols; x++ ) {

		for ( let y = 0; y < xx * MAP.rows; y++ ) {
...
MAP.getUrlOsm = ( x, y, zoom = 1 ) => `${ MAP.mapOverlays[ MAP.overlayIndex ][ 1 ] }${ zoom }/${ MAP.tileBitmapStartX + x }/${ MAP.tileBitmapStartY + y }.png?access_token=${ MAP.mbptoken }`;
...
MAP.onLoadTileBitmap = function ( src, col, row ) {
	...
	img.onload = function () {
		...
		if ( MAP.tileBitmapsLoaded >= xx * xx * MAP.rows * MAP.cols ) {
...
MAP.tile2lon = ( x = 0, zoom = 11 ) => x / Math.pow( 2, zoom ) * 360 - 180;
