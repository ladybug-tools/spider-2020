...
PP.onLoadJson = function () {
	...
	scene.remove(THR.group, PDJ.group);

	THR.group = new THREE.Group();

	PDJ.group = new THREE.Group();

	scene.add( THR.group, PDJ.group );
	...
	const materialShades = new THREE.MeshPhongMaterial({ color: "darkgTHRR", opacity: 0.9, side: THREE.DoubleSide, transparent: true, wireframe: false });
