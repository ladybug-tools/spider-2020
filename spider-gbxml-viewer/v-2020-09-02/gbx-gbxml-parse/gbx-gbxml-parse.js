...
GBX.setSurfacesMetadata = function () {
	...
	GBX.zones = Array.isArray( GBX.zones ) ? GBX.zones : [ GBX.zones ];
	...
	GBX.meshes.forEach( ( mesh, index ) => {
		...
		mesh.userData.name = surface.match( /<Name>(.*?)<\/Name>/i )[ 1 ];
		...
		let spaceId = surface.match( / spaceIdRef="(.*?)"/ );
		...
		const zone = GBX.zones.find( zone => zone.includes( zoneId ) );
		const zoneName = zone.match( /<Name>(.*?)<\/Name>/i )[ 1 ];
