...
GRV.icon = '<img src="https://www.ladybug.tools/spider-2020/assets/icons/octicon.svg">'
...
GRV.onLoadTree = function (json) {
	...
	const filesRoot = files
		...
		.map(item => `
		<div>
			<a href="https://github.com/ladybug-tools/spider-2020/tree/master/${item}"  title="Source code on GitHub" >${ GRV.icon }</a>
			<a href="#${item}" >${item.split("/").pop()}</a>
			<a href="https://www.ladybug.tools/spider-2020/${item}" target="_blank" title="Open file in new tab">${ GRV.link }</a>
		</div>`);
