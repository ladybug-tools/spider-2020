...
GRV.xxxonLoadTree = function ( json ) {
	...
	paths = tree.filter( obj => obj.type === "tree" )
		.map( item => item.path );
	...
	for ( path of paths ) {
		...
		for ( let ignore of GRV.ignoreFolders ) {
			...
			if ( path.startsWith( ignore ) ) { count++ }
	...
	//subtrees = paths.filter( item => !GRV.ignoreFolders.includes( item ) )
	subtrees = paths.map( subtree => subtree.split( "/" ) );
